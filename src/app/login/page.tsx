...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    try {
      // In production: await signIn(email, password);
        ...
        <form onSubmit={handleSubmit} className="bg-card-bg border border-card-border rounded-xl p-6 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button type="button" className="text-xs text-accent hover:underline">Vergessen?</button>
            </div>
            <div className="relative">
          ...
          <button type="button"
            className="w-full bg-input-bg border border-card-border hover:border-muted text-foreground font-medium py-2.5 rounded-lg transition-all text-sm flex items-center justify-center gap-2">
