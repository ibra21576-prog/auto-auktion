...
import { FiPlus, FiEdit2, FiEye, FiClock, FiCheckCircle, FiXCircle, FiTrendingUp, FiDollarSign, FiList } from 'react-icons/fi';
...
export default function HaendlerDashboard() {
  ...
  useEffect(() => {
    // In production: load from Firestore filtered by sellerId
    setAuctions(mockAuctions.slice(0, 3));
  }, []);
                    ...
                    <tr key={a.id} className={`${i < auctions.length - 1 ? 'border-b border-card-border' : ''} hover:bg-input-bg/50 transition-colors`}>
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-lg overflow-hidden bg-input-bg flex-shrink-0">
                      ...
                      <td className="px-5 py-3 text-muted">{a.bidCount}</td>
                      <td className="px-5 py-3 text-muted text-xs">
                        {new Date(a.endTime).toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}
                      </td>
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          {a.status === 'draft' && (
                            <Link href={`/erstellen?edit=${a.id}`} className="p-1.5 text-muted hover:text-accent transition-colors" title="Bearbeiten">
